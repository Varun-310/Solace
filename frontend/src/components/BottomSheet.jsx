 ...
 * Mobile slide-up panel with conversation history, custom modals.
 */
...
const BottomSheet = ({ isOpen, onClose, onNewChat, onClearHistory, conversations = [], onLoadConversation, onDeleteConversation }) => {
            ...
            >
                ...
                <div className="px-5 pb-6 overflow-y-auto" style={{ maxHeight: 'calc(85vh - 24px)' }}>
                    ...
                    <div className="mb-5 p-4 rounded-2xl"
                        ...
                        {isAuthenticated ? (
                            <div className="flex items-center gap-3.5">
                                ...
                                <button onClick={onClose} className="p-2 rounded-xl" style={{ color: 'var(--color-text-muted)' }}>
                                    <X className="w-5 h-5" />
                                </button>
                            </div>
                        ) : (
                    ...
                    <div className="grid grid-cols-2 gap-2.5 mb-5">
                        <ActionCard icon={Plus} label="New Chat" sublabel="Start fresh" color="#3A7D5C" onClick={() => { onNewChat?.(); onClose(); }} />
                        <ActionCard icon={Settings} label="Settings" sublabel="Preferences" color="#C8956C" onClick={() => handleNav('/settings')} />
                        <ActionCard icon={Info} label="About" sublabel="Learn more" color="#7B6BA0" onClick={() => handleNav('/about')} />
                        {isAuthenticated && user?.email?.toLowerCase() === ADMIN_EMAIL && (
                            <ActionCard icon={Shield} label="Admin" sublabel="Dashboard" color="#2B5E44" onClick={() => handleNav('/s0lace-ctrl')} />
                        )}
                    </div>
                        ...
                        <div className="mb-5 pt-4 border-t" style={{ borderColor: 'var(--color-border)' }}>
                            ...
                            <div className="space-y-1">
                                {conversations.slice(0, 5).map((conv) => (
                                    <div key={conv.session_id} className="relative group">
                                        <button
                                        ...
                                        <button
                                            onClick={(e) => {
                                                ...
                                                showModal({
                                                    ...
                                                    message: "This will permanently remove this conversation. This cannot be undone.",
                                                    confirmLabel: "Delete",
                                                    variant: "danger",
...
const ActionCard = ({ icon: Icon, label, sublabel, color, onClick }) => (
    <button onClick={onClick}
        ...
        onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = 'var(--shadow-medium)'; e.currentTarget.style.borderColor = color; }}
